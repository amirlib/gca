...
class bfsGraph extends Graph {
    ...
    addLayer(index) {
        ...
        if (index > this.layer.length) {
            throw new layerError(`Some layers before layer number: ${index}, not exists.`);
        }
        this.layer[index] = [];
    }
    /**
    ...
    getNodeIDFromLayer(cellLayer, indexLayer) {
        ...
        if (cellLayer >= this.layer[indexLayer].length) {
            throw new layerError(`There is no node in cell: ${cellLayer} at layer number: ${indexLayer}.`);
        }
